...
class PortfolioManager:
    ...
    def get_last_buy_time(self) -> Optional[str]:
        """Get the last buy time for cooldown calculation."""
        return self.portfolio.get("last_buy_time")
    ...
    def buy(self, signal_type: str, expiry: str, strike: float, ltp: float, snapshot_id: int, snapshot_seq: int) -> Tuple[bool, str]:
        ...
        # Track last buy snapshot and time for cooldown calculation
        self.portfolio["last_buy_snapshot_seq"] = snapshot_seq
        self.portfolio["last_buy_time"] = datetime.now().isoformat()
